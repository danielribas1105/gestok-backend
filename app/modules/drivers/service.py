...
async def list_drivers(offset: int = 0, limit: int = 20) -> list[Driver]:
    result = await db.session.execute(
        select(Driver).offset(offset).limit(limit).order_by(Driver.name)
    )
...
async def create_driver(data: DriverCreate) -> Driver:
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail="Motorista já cadastrado.",
        ) from e
    return await get_driver_by_id(driver.id)
...
async def update_driver(driver_id: uuid.UUID, data: DriverUpdate) -> Driver:
...
async def delete_driver(driver_id: uuid.UUID) -> None:
    driver = await get_driver_by_id(driver_id)
    if not driver:
        raise HTTPException(status_code=404, detail="Motorista não encontrado")
